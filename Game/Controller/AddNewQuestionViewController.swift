...
class AddNewQuestionViewController: UIViewController {
    ...
    private func setupQuestionTextField() {
        ...
        questionTextField.textColor = Color.text
        questionTextField.attributedPlaceholder = NSAttributedString(string: "Текст вопроса...",
                                                                     attributes: [NSAttributedString.Key.foregroundColor: Color.text])
        questionTextField.addBottomLine(with: Color.scoreBackground)
    }
    ...
    private func setupAnswerTextFields() {
        ...
        for (index, textField) in answerTextFields.enumerated() {
            ...
            textField.attributedPlaceholder = NSAttributedString(string: "Ответ №\(index+1)",
                                                                         attributes: [NSAttributedString.Key.foregroundColor: Color.text])
            textField.addBottomLine(with: Color.scoreBackground)
        }
    }
    ...
    private func setupCorrectAnswerSegmentedControl() {
        ...
        correctAnswerSegmentedControl.backgroundColor = Color.elementBackground
        correctAnswerSegmentedControl.selectedSegmentTintColor = Color.scoreBackground
        correctAnswerSegmentedControl.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Color.textAlternative], for: .selected)
        correctAnswerSegmentedControl.setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Color.text], for: .normal)
